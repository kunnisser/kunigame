...
class Loading extends KnScene {
  ...
  dev() {
    if (
      ...
      this.game.gui.__controllers[0].property === "加载类型"
    ) {
      return;
    }
    ...
    this.dat = this.game.gui.add(dat, "加载类型", [
      "mask",
      "particle",
      "sprite"
    ]);
  ...
  generateBar() {
    ...
    const maskClip = drawStage.generateRect(
      0x000000,
      [0, 0, innerBar.height + 2, innerBar.width],
      !0
    );
  ...
  generateSprite() {
    ...
    const drawStage = this.game.add.graphics();
    const loadingbar = drawStage.generateRect(
      0x337ab7,
      [0, 0, this.game.config.half_w + anmi.width, 10, 5],
      !0
    );
    loadingbar.y = anmi.height * 0.5 - 20;
    this.loadingGp.addChild(loadingbar);
    const startX = -loadingbar.width * 0.5 + anmi.width;
    anmi.x = startX;
    // 绘制加载文字
    const loadingText = this.game.add.text(
      "",
