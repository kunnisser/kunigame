 ...
 * @LastEditTime: 2021-01-21 16:00:43
 * @FilePath: /kunigame/kuni/src/state/postion/index.ts
...
class Triangulation extends KnScene {
  ...
  create() {
    ...
    this.addBackground();
    let r1 = 100 + Math.random() * 100;
    let r2 = 30 + Math.random() * 100;
    ...
    let c1 = this.game.add.graphics().generateLineCircle(0xd10311, [point1.x, point1.y, r1]);
    let c2 = this.game.add.graphics().generateLineCircle(0xe8a91a, [point2.x, point2.y, r2]);
    let c3 = this.game.add.graphics().generateLineCircle(0x1a73e8, [point3.x, point3.y, r3]);
    ...
    // 质心加权
    let x = (retPoint1.x / (r1 + r2) + retPoint2.x / (r1 + r3) + retPoint3.x / (r2 + r3)) / (
      1 / (r1 + r2) + 1 / (r1 + r3) + 1 / (r2 + r3)
    );
    let y = (retPoint1.y / (r1 + r2) + retPoint2.y / (r1 + r3) + retPoint3.y / (r2 + r3)) / (
      1 / (r1 + r2) + 1 / (r1 + r3) + 1 / (r2 + r3)
    );
    // 绘制定位点
    ...
    // 优化的三点质心加权
    let x1 = (retPoint1.x * (1 / r1 + 1 / Math.pow(r2, r1 / r2))
    ...
    let y1 = (retPoint1.y * (1 / r1 + 1 / Math.pow(r2, r1 / r2))
  ...
  computedPointer(r1, r2, p1, p2, rest) {
    let AB: number = Math.sqrt(Math.pow(p1.x - p2.x, 2) + Math.pow(p1.y - p2.y, 2));
    console.log(AB);

    if (AB < r1 + r2 && Math.abs(r1 - r2) < AB) {
      console.log('相交');
      let AE: number = (Math.pow(r2, 2) - Math.pow(r1, 2) - Math.pow(AB, 2)) / (-2 * AB);
      console.log(AE);
      ...
      let restPointC = Math.abs(pointC.x - rest.x) + Math.abs(pointC.y - rest.y);
      let restPointD = Math.abs(pointD.x - rest.x) + Math.abs(pointD.y - rest.y);

      if (restPointC > restPointD) {
        let pd = this.game.add.graphics().generateCircle(0xffffff, [pointD.x, pointD.y, 6]);
        this.addChild(pd);
        return pointD;
      } else {
        // 绘制相交点
        let pc = this.game.add.graphics().generateCircle(0xffffff, [pointC.x, pointC.y, 6]);
        this.addChild(pc);
        return pointC;
      }
    } else if (AB == r1 + r2) {
      ...
      console.log(pointE);
      // 绘制相交点
      let pe = this.game.add.graphics().generateCircle(0xffffff, [pointE.x, pointE.y, 6]);
      this.addChild(pe);
      return pointE;
    } else if (AB > r1 + r2) {
      console.log('相离');
