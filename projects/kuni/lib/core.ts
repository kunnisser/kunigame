...
export default class Game {
  ...
  constructor(config: EnterProps) {
    ...
    PIXI.settings.RESOLUTION = this.dpr;
    window["PIXI"] = PIXI;
    this.camera = {};
    config.ratio
    ...
    // 设置游戏画布基本尺寸
    const _gameWidth: number = config.editorWidth || config.width;
    const _gameHeight: number = config.editorHeight || config.height || 0;
    this.config = {
      width: _gameWidth,
      height: _gameHeight,
      half_w: _gameWidth * 0.5,
      half_h: _gameHeight * 0.5,
      editorWidth: _gameWidth,
      editorHeight: _gameHeight,
