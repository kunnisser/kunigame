...
class KnScrollMenu extends KnGroup {
  ...
  constructor(game: Game, parent: Container, options: any, anchor?: boolean, interval?: number, cb?: Function) {
  ...
  buildMenu(opt: any) {
    ...
    const menuName = this.game.add.section(opt.name, '', Math.round(menuIcon.width / 12), menu, {
      padding: 20,
      bg: 0xe5b240
    });
